 ...
 * @author Eli Lipsitz
 */
public class WednesdayPlayer {
    private Sequence sequence = null;
    private Cello[] strings;
    private double[] vol;

    public WednesdayPlayer(InputStream source) {
        try {
    ...
    public WednesdayPlayer(File source) {
        try {
            sequence = MidiSystem.getSequence(source);
        } catch (IOException | InvalidMidiDataException e) {
            e.printStackTrace();
    ...
    private void initialize() {
        strings = new Cello[128];
        vol = new double[128];
        for (int i = 0; i < strings.length; i++) {
            strings[i] = new Cello(410.0 * Math.pow(2.0, (i - 69.0) / 12.0));
            vol[i] = 0.0;
        }
    }
